...
export async function analyzeSymptoms({ organ, selectedSymptoms, allSymptoms, imageBase64 }) {
  const token = await AsyncStorage.getItem('triacare_token');
...
|---|---|---|---|---|
| `paymentService.js:21,30` | `triacare_wallet_<uid>` (per user) | **scans** (`balanceScans`) | UGX 500 / $0.50 | 1/5/10/20 |
| `walletService.js:10,40` | `triacare_wallet` (global) | **dollars** (`balance`) | $0.50 | 2/6/12/30 |
